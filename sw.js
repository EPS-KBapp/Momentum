...
const ASSETS = [
  ...
  // ⚠️ N'ajoute ici QUE les fichiers qui existent réellement sur ton serveur
];
      ...
      caches.open(CACHE_NAME).then(cache =>
        cache.match(event.request).then(cached => {
          const network = fetch(event.request).then(resp => {
            cache.put(event.request, resp.clone()); // ✅ clone avant mise en cache
            return resp;
          });
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(resp => {
        // ✅ On vérifie que la réponse est valide avant de cloner
        if (!resp || resp.status !== 200 || resp.type === 'opaque') return resp;

        const toCache = resp.clone(); // ✅ clone AVANT de retourner resp
